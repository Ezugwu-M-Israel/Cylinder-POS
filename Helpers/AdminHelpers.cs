...
namespace GasPOS.Helpers
{
	public class AdminHelpers : IAdminHelpers
	{
		...
		public List<CynlinderCategoryViewModel> GetListOfCynlinderCategories()
		{
			...
			var list = _context.CynlinderCategories.Where(x => x.Id != 0 && x.Active && !x.Deleted).ToList();
		...
		public CynlinderCategory GetCynliderCategoryByCynliderId(int id)
		{
			...
			if (id != 0)
			{
				var category = _context.CynlinderCategories.Where(x => x.Id == id && x.Name != null && x.Active && !x.Deleted).FirstOrDefault();
				if (category != null)
				{
					return category;
		...
		public CynlinderViewModel GetCynlinderByCynlinderId(int id)
		{
			...
			if (id != 0)
			{
				var category = _context.Cynlinders.Where(x => x.Id == id && x.Name != null && x.Active && !x.Deleted).Include(x => x.CynlinderCategory).FirstOrDefault();
				if (category != null)
				{
		...
		public async Task<List<CynlinderCategory>> GetCynlinderCategory()
		{
			try
			{
				...
				var listOfCynlinderCategorys = await _context.CynlinderCategories.Where(a => a.Id != 0 && !a.Deleted && a.Active).OrderBy(m => m.Name).ToListAsync();
		...
		public List<OrderViewModel> ListOfOrder()
		{
			...
			var list = _context.Orders.Where(o => o.Id != 0 && o.Active && !o.Deleted).ToList();
			if (list != null && list.Count > 0)
			{
				foreach (var lists in list)
				{
			...
			try
			{
				if (cynlinderCategoryViewModel != null)
				{
					...
					if (cynlinderCategoryDetails != null)
					{
						...
						cynlinderCategoryDetails.Deleted = false;
